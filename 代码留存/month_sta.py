...
for yy in range(15,16):
    #####TRMM每天的降雨值#####
    arcpy.env.workspace = "F:/SA/RG/IMERG/M/"
...
#        print (length,gauges)
    ...
    for month in range(1, length):
        TRMM_DBF_Table[month] = DBF(
            "F:/SA/RG/IMERG/M/" + str((2000+yy)*100+month) + "_RG.dbf",
            load=True)
    TRMM_3B42_RG_Table = {}
    for month in range(1, length):
...
#              print("分母有0值")
            ...
            # print (str(R[month]))
    sta = csv.writer(open("F:/SA/RAIN/STA/MY/"+str(2000+yy)+"_stam.csv", "w"))
    ...
    #保存数据
    staKL = csv.writer(open("F:/SA/RAIN/STA/MY/"+str(2000+yy)+"_KLm.csv", "w"))
    ...
    # print(Ratio_SYD)
    #保存数据
    staS = csv.writer(open("F:/SA/RAIN/STA/MY/"+str(2000+yy)+"_SYDm.csv", "w"))
    staS.writerow(['SYD'])
