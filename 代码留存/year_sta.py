...
for year in range(1, length):
    TRMM_DBF_Table[year] = DBF(
        "F:/SA/RG/IMERG/Y/" + str(2014+year) + "_RG.dbf",
        load=True)
TRMM_3B42_RG_Table = {}
for year in range(1, length):
...
#####站点每天的降雨值#####
RG_DBF_Table = DBF("F:/SA/RAIN/RG_dbf/MY/ym_RG.dbf", load=True)
RG_Table = {}
for year in range(1, length):
    RG_Table[year] = [0 for x in range(0, gauges)]  # 站点的数量200812与201007不一样
for year in range(1, length):
    for j in range(0, gauges):
        RG_Table[year][j] = RG_DBF_Table.records[j]["Y"+str(2014+year)]
#        print(RG_Table)
#####R2，RMSE，MAE，Bias#####
TRMM_Data_Average = {}
RG_Data_Average = {}
...
#              print("分母有0值")
        ...
        # print (str(R[year]))
sta = csv.writer(open("F:/SA/RAIN/STA/MY/stay.csv", "w"))
sta.writerow(['Year','R','RMSE','Bias','MAE'])
for year in range(1,length):
    sta.writerow([str(2014+year),R[year],RMSE[year],Bias[year],MAE[year]])
...
#
# #保存数据
# staKL = csv.writer(open("F:\\Research\\RainGauge\\Statistics\\year\\KL_y.csv", "w"))
...
# print(Ratio_SYD)
#保存数据
staS = csv.writer(open("F:\\Research\\RainGauge\\Statistics\\year\\SYDy.csv", "w"))
staS.writerow(['SYD'])
