...
for yy in range(15,16):
    TRMM={}
    RG = {}
    TRMM= [1 for x in range(1, 13)]
    RG = [1 for x in range(1, 13)]
    staI = csv.writer(open("F:/SA/RAIN/STA/" + "ID.csv", "w"))
    staR = csv.writer(open("F:/SA/RAIN/STA/" + "RD.csv", "w"))
    for mm in range(1,13):
        #####TRMM每天的降雨值#####
        arcpy.env.workspace = "F:/SA/RG/IMERG/D/"+str(i)+"/"
...
#        print (length,gauges)
        ...
        TRMM[mm]= [1 for x in range(1, length)]
        RG[mm] = [1 for x in range(1, length)]

        TRMM_DBF_Table = {}
        for Daily in range(1, length):
            TRMM_DBF_Table[Daily] = DBF(
                "F:/SA/RG/IMERG/D/"+str(i)+"/"+str(((2000+yy)*100+mm)*100+Daily) + "_RG.dbf",load=True)
...
#        print(TRMM_3B42_RG_Table,'\n')
        ...
        #####站点每天的降雨值#####
        RG_DBF_Table = DBF("F:/SA/RAIN/RG_dbf/D/" + str((2000+yy)*100+mm) + ".dbf", load=True)
        RG_Table = {}
        for Daily in range(1, length):
            RG_Table[Daily] = [0 for x in range(0, gauges)]  # 站点的数量200812与201007不一样
        for Daily in range(1, length):
            for j in range(0, gauges):
                RG_Table[Daily][j] = RG_DBF_Table.records[j]["D" + str(Daily)]
#        print(RG_Table)
        #####R2，RMSE，MAE，Bias#####
        TRMM_Data_Average = {}
        RG_Data_Average = {}
        ...
        for Daily in range(1, length):
            ...
            TRMM[mm][Daily]=TRMM_Data_Average[Daily]
            RG[mm][Daily] = RG_Data_Average[Daily]
            ...
            if (sum2[Daily] == 0.0 or sum3[Daily] == 0.0 or sum4[Daily] == 0.0 or sum7[Daily] == 0.0):
  #              print("分母有0值")
                R[Daily] = 999999
                ...
                # print (str(R[Daily]))
        sta = csv.writer(open("F:/SA/RAIN/STA/D/"+str((2000+yy)*100+mm)+"_stad.csv", "w"))
        ...
        #保存数据
        staKL = csv.writer(open("F:/SA/RAIN/STA/D/"+str((2000+yy)*100+mm)+"_KLd.csv", "w"))
        ...
        # print(Ratio_SYD)
        #保存数据
        staS = csv.writer(open("F:/SA/RAIN/STA/D/"+str((2000+yy)*100+mm)+"_SYDd.csv", "w"))
        staS.writerow(['SYD'])
        for i in range(1, length):
            staS.writerow(Ratio_SYD[i])

    for m in range(1, 13):
        staI.writerow(TRMM[m])
        staR.writerow(RG[m])
